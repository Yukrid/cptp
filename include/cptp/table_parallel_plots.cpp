...
namespace cptp{
    ...
    void TabParPlots::_decideRange(void) noexcept
    {
        ...
        if(range.x.min==f_max || range.x.max==f_min){
            ...
            for(auto& data_array : _data_set.range()){
                for(auto& d2 : data_array.range()){
                    for(auto& x : d2.datas().path<0>().range()){
        ...
        if(range.y.min==f_max || range.y.max==f_min){
            ...
            for(auto& data_array : _data_set.range()){
                for(auto& d2 : data_array.range()){
                    for(auto& y : d2.datas().path<1>().range()){
    ...
    void TabParPlots::_drawBox(const Vec2& fig_, std::ofstream& output_, Vec2& scaler_) noexcept
    {
        ...
        region.y=fig_.y-pointToCenti(label.x.point+scale.x.point   )*1.1-label.margin.y-figure.margin.y*2.-0.1;
    ...
    void TabParPlots::_drawScale(std::ofstream& output_, Vec2& sca_, Range2D& r2d_) noexcept
    {
        switch(scale.x.arg.number()){
            ...
            case Scale::DECIMAL:
            {
                ...
                if(width>30.0) while(width>30.0){ width*=0.1;  exp*=0.1;  ++count; }
