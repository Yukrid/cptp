...
int main(void){
    ...
    {
        // ファイルストリームを利用
        std::ifstream ifs1("../src/example2/ydata00_1.txt");
        std::ifstream ifs2("../src/example2/ydata00_2.txt");
    ...
    cptp::Data2DArray data_array01;
    {
        {
            // ファイルストリームを利用
            std::ifstream ifs1("../src/example2/ydata01.txt"); 
    ...
    // figureサイズを縦長に変更
    tpp.figure.size.construct(cptp::Figure::CAMMPUS, cptp::Vec2(8., 10.));

    // tex/example1.texとしてtexファイルを作成
    std::filesystem::create_directory("example2_tex");
    tpp.outputPlot("example2_tex/example2.tex");
}
