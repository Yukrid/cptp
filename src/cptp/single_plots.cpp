...
namespace cptp{
    ...
    //    Function Definition    //
    //---------------------------//
    //(    cptp::SinglePlots Strcuture   )//
    //-> forward.hpp table_parallel_plots.hpp plot_spec.cpp
    //_ Constructor
    SinglePlots::SinglePlots(uint16_t p_) noexcept
        :  figure          ( )
        ,  label           ( )
        ,  scale           ( )
        ,  key             ( )
    ...
    SinglePlots& SinglePlots::construct(uint16_t p_) noexcept
    {
        new(this) SinglePlots(p_);
        
        return *this;
    }
 

    SinglePlots& SinglePlots::reconstruct(uint16_t p_) noexcept
    {
        this->~SinglePlots();
        new(this) SinglePlots(p_);
    ...
    //_ Destructor
    SinglePlots& SinglePlots::destruct (void) noexcept
    {
        this->~SinglePlots();
    ...
    //_ Accessor
    Title& SinglePlots::title(void) noexcept
    {
        return _title_set[0];
    }
    ...
    Data2DArray& SinglePlots::data(void) noexcept
    {
        return _data_set[0];
    }
    ...
    PlotSpec& SinglePlots::plot_spec(uint16_t p_) noexcept
    {
    ...
    //_ Writer
    void SinglePlots::info(std::ostream& os_, uint_fast8_t offset_) const noexcept
    {
        std::string str;
        for(uint_fast8_t a=0; a<offset_; ++a) str+="  ";
        
        os_<<str<<"[cptp::SinglePlots Class Template]"<<std::endl;
        os_<<str<<"(size)                   : "<<sizeof(SinglePlots)<<std::endl;
        os_<<str<<"(name)                   : "<<typeid(SinglePlots).name()<<std::endl;
    ...
    //_ Variable Function
    void SinglePlots::setPlotSpec(uint16_t p_, const PlotSpec& ps_) noexcept
    {   
    ...
    void SinglePlots::setDatas(const Data2DArray& ds_, const Title& tit_) noexcept
    {
        _title_set[0]=tit_;
        _data_set [0]=ds_;
    ...
    void SinglePlots::outputPlot(const std::string& str_) noexcept
    {
    ...
    void SinglePlots::_writeInitialCode(std::ofstream& output_, Vec2& fig, Vec2& camp) noexcept
    {
    ...
    void SinglePlots::_decideRange(void) noexcept
    {
    ...
    void SinglePlots::_drawBox(const Vec2& fig_, std::ofstream& output_, Vec2& scaler_) noexcept
    {
        Vec2 region, base;
    ...
    void SinglePlots::_drawScale(std::ofstream& output_, Vec2& sca_, Range2D& r2d_) noexcept
    {
        switch(scale.x.arg.number()){
            ...
            case Scale::DECIMAL:
            {
                ...
                if(0.0005<exp && exp<5000.){
                    ...
                    output_<<"\\draw ($(BOX_LB_"<<0<<"_"<<0<<")+(BOX_REGION_X)-(0,"<<pointToCenti(scale.x.point)*1.5<<")$) node[below] {\\fontsize{"<<scale.x.point<<"pt}{0pt}\\selectfont $\\times 10^{"<<count<<"}$};"<<std::endl;
                }
                {
        ...
        switch(scale.y.arg.number()){
            case Scale::DECIMAL:
            {
                ...
                if(0.0005<exp && exp<5000.){
                    ...
                    output_<<"\\draw ($(BOX_LB_"<<0<<"_"<<0<<")+(BOX_REGION_Y)+(0, "<<pointToCenti(scale.y.point)*0.5<<")$) node[above] {\\fontsize{"<<scale.x.point<<"pt}{0pt}\\selectfont $\\times 10^{"<<count<<"}$};"<<std::endl;
                }
    ...
    void SinglePlots::_drawLabel(std::ofstream& output_) noexcept
    {
    ...
    void SinglePlots::_drawPlot(const Vec2& sca_, const Range2D& r2d_, std::ofstream& output_, gd::List<gd::Group<uint32_t, 4>>& cofs_) noexcept
    {
    ...
    void SinglePlots::_drawKey(const gd::List<gd::Group<uint32_t, 4>>& cofs_, std::ofstream& output_, uint32_t key_nmpos_[2]) noexcept
    {
        size_t   index=0;
    ...
    void SinglePlots::_drawTitle(const gd::List<gd::Group<uint32_t, 4>>& cofs_, const uint32_t key_nmpos_[2], std::ofstream& output_) noexcept
    {
    ...
    void SinglePlots::_writeFinalCode(std::ofstream& output_) noexcept
    {
    ...
    //_ Constant Function
    size_t SinglePlots::_memory(void) const noexcept
    {
        return 0;
    }
